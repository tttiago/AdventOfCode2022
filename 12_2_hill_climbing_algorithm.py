...
y_possible_start, x_possible_start = np.where(heights == 0)

trail_distances = {}
for (x_start, y_start) in zip(x_possible_start, y_possible_start):
    trail_distances[x_start, y_start] = shortest_path(graph, (x_start, y_start), (x_end, y_end))

print(min([x for x in trail_distances.values() if x >= 0]))
